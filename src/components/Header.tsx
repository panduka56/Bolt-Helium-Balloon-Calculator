...
import { Moon as Balloon, Menu, X, Phone, ShoppingCart, Award, Truck, HeadphonesIcon } from 'lucide-react';
import { Link, useLocation } from 'react-router-dom';

const NAV_LINKS = [
  { name: 'Calculator', to: '/', description: 'Free helium calculator' },
  { name: 'Helium Products', to: '/products', description: 'Browse cylinders & accessories' },
  { name: 'Delivery Info', to: '/delivery', description: 'UK-wide delivery options' },
...
const TRUST_INDICATORS = [
  ...
  { icon: HeadphonesIcon, text: 'Expert Support', subtitle: 'Call 0800 195 4445' },
];
  ...
  return (
    <>
      ...
      >
        ...
        <div className="bg-gradient-to-r from-orange to-pink text-white text-center py-1 text-xs font-medium">
          🎈 Free UK delivery on orders over £50 | Order by 2PM for next-day delivery
        </div>
        ...
        <div className="bg-gradient-to-b from-gray-50 to-white border-b border-gray-100">
          <div className="max-w-[1400px] mx-auto px-4 lg:px-8">
            <div className="flex items-center justify-between py-3 lg:py-4">
              ...
              >
                <span className="inline-flex items-center justify-center h-12 w-12 rounded-full bg-gradient-to-br from-orange/20 to-pink/20 group-hover:from-orange/30 group-hover:to-pink/30 transition-all shadow-sm">
                  <Balloon className="h-7 w-7 text-orange group-hover:scale-110 transition-transform" />
                </span>
                <div>
              ...
              {/* Center: Navigation (Desktop) - with better spacing */}
              <nav className="hidden lg:flex items-center justify-center flex-1 px-8" aria-label="Main navigation">
                <div className="flex items-center gap-x-1 xl:gap-x-2 bg-gray-50/50 rounded-full px-4 py-2">
                  {NAV_LINKS.map(link => {
                    ...
                    return (
                      <Link 
                        key={link.name} 
                        to={link.to} 
                        className={`relative px-4 xl:px-6 py-2.5 font-semibold rounded-full transition-all duration-200 group ${
                          isActive 
                            ? 'text-white bg-gradient-to-r from-orange to-pink shadow-md' 
                            : 'text-gray-700 hover:text-orange hover:bg-white hover:shadow-sm'
                        }`}
              ...
              <div className="hidden lg:flex items-center gap-x-3 xl:gap-x-4 flex-shrink-0">
                ...
                <a 
                  ...
                  className="flex items-center gap-x-2 bg-gradient-to-r from-orange to-pink text-white px-5 xl:px-6 py-2.5 rounded-full font-bold shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200"
                ...
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center space-x-3">
                    <span className="inline-flex items-center justify-center h-10 w-10 rounded-full bg-orange/20">
                      <Balloon className="h-6 w-6 text-orange" />
                    </span>
                    <div>
                ...
                <div className="border-t pt-6 space-y-3">
                  {TRUST_INDICATORS.map((indicator, index) => (
                    <div key={index} className="flex items-center space-x-3 text-sm">
                      ...
                      <div>
                        <div className="font-semibold text-dark">{indicator.text}</div>
                        <div className="text-gray-500 text-xs">{indicator.subtitle}</div>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="hidden lg:block bg-gradient-to-r from-pink/20 to-orange/20 border-t border-pink/30">
          <div className="max-w-7xl mx-auto px-4 py-2">
            <div className="flex justify-center items-center space-x-8 text-sm">
              {TRUST_INDICATORS.map((indicator, index) => (
                <div key={index} className="flex items-center space-x-2 text-dark">
                  <indicator.icon className="h-4 w-4 text-orange" />
                  <span className="font-medium">{indicator.text}</span>
                  <span className="text-gray-600 hidden xl:inline">• {indicator.subtitle}</span>
                </div>
              ))}
            </div>
          </div>
