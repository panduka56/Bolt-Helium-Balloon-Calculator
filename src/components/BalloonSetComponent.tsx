...
export const BalloonSetComponent: React.FC<BalloonSetComponentProps> = ({
  ...
  const updateBalloon = (balloon: Balloon) => {
    const existingIndex = balloonSet.balloons.findIndex(
      (b) => b.size === balloon.size,
    );
  ...
  return (
    <div className="p-6 border border-pink rounded-2xl bg-white transition-all shadow-md hover:shadow-lg">
      <div className="flex justify-between items-center mb-4">
        {setNumber > 1 && (
          <h4 className="font-bold text-dark">{`Balloon Set ${setNumber}`}</h4>
        )}
        {showRemoveButton && (
          <button
            onClick={onRemove}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {balloonSizes.map((sizeOption) => {
          const existingBalloon = balloonSet.balloons.find(
            (b) => b.size === sizeOption.size,
          );
